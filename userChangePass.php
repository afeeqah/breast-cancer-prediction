<?php
// Include userNavbar.php for navigation
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($currentPasswordErr) && empty($newPasswordErr) && empty($confirmPasswordErr)) {
        // Retrieve the current password associated with the user from the database
        $username = $_SESSION['user_username'];
        $query = "SELECT password FROM users WHERE user_username='$username'";
        ...
        if ($currentPassword == $currentPasswordFromDB) {
            // Update the password
            $updateQuery = "UPDATE users SET password='$newPassword' WHERE user_username='$username'";
